...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.685\textwidth]{assets/pca.png}
...
\begin{enumerate}
    \item \textbf{Instance-Specific Fitting}: Unlike neural networks that learn a global function across all patients, AR models fit coefficients independently for each signal. This avoids the need for a large training dataset and eliminates generalization issues across the different anatomies of patients.
    \item \textbf{Prediction Error as a Biomarker}: The prediction error itself can be a potential biomarker. Arrhythmic tissue is expected to be harder to predict than healthy tissue. This approach favors more simple models in comparison to complex predictors.
    \item \textbf{Interpretability}: AR coefficients have direct physical meaning related to tissue properties, whereas neural network weights are less explainable.
    \item \textbf{Computational Efficiency}: AR models fit instantaneously via closed-form solutions (OLS), enabling real-time analysis of thousands of electrodes without GPU resources.
\end{enumerate} For this approach, we assume that $\mathbb{E}[X_t^2] < \infty$. This condition ensures that the second-order moments required for linear prediction exist.

The justification for this model comes from Wold's Decomposition Theorem \cite{brockwell1991time}. Any zero-mean, non-deterministic stationary time series $X_t$ can be expressed as an infinite moving average representation:
\begin{equation}
...
\end{equation}
The coefficients $\boldsymbol{\phi}_t$ are estimated recursively using Recursive Least Squares (RLS) via the Kalman Filter \cite{Kalman_1960}. This allows the model to adapt to changing conditions (e.g., heart rate acceleration).
...
\begin{itemize}
    ...
    \item \textbf{Soft Dynamic Time Warping (SoftDTW) Loss}: We also explored SoftDTW \cite{cuturi2018softdtwdifferentiablelossfunction}, a differentiable variant of Dynamic Time Warping that measures similarity between time series while allowing for temporal shifts. This metric was promising, as it penalizes phase errors less harshly than MSE. However, it was computationally infeasible for our dataset due to its $O(N \cdot M)$ complexity per signal pair.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{High NMSE} ($> 1$): Complex, unpredictable tissue. The AR model fails, suggesting Complex Fractionated Atrial Electrograms (CFAEs) or drivers that are potential ablation targets.
\end{itemize}
...
\begin{itemize}
    ...
    \item A clear \textbf{high-error tail} identifies \textbf{anomaly candidates}. These are signals significantly more unpredictable than the rest.
...
Based on the NMSE ranking, we identified the top 10\% of electrodes by prediction error as \textbf{candidate ablation sites}. These regions, distributed across multiple patients and maps, represent areas where the AR model fails most severely and are therefore hypothesized to correspond to active arrhythmic drivers.

Table~\ref{tab:ablation_candidates} presents a sample of the most unpredictable (potential arrhythmic drivers) and most predictable (healthy tissue) electrodes from the test set, after excluding three extreme outliers caused by numerical instability in the AR fitting process.

\begin{table}[H]
\centering
\caption{Top 5 most unpredictable and most predictable electrodes by NMSE.}
\label{tab:ablation_candidates}
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Patient Date} & \textbf{Map Name} & \textbf{Electrode} & \textbf{NMSE} & \textbf{MSE} \\
\hline
\multicolumn{5}{|c|}{\textbf{Most Unpredictable (Potential Arrhythmic Drivers)}} \\
\hline
2021\_10\_19 & 1-AI & 0 & 19.55 & 0.0451 \\
2021\_11\_17 & 1-AI BASAL FA & 244 & 18.28 & 0.0024 \\
2021\_11\_08 & 1-AI & 445 & 18.04 & 0.0003 \\
2021\_11\_08 & 1-AI & 4517 & 17.61 & 0.1038 \\
2021\_10\_18 & 1-1-1-FA & 409 & 14.44 & 0.0001 \\
\hline
\multicolumn{5}{|c|}{\textbf{Most Predictable (Healthy / Passively Activated Tissue)}} \\
\hline
2021\_11\_08 & 1-AI & 2567 & 0.178 & 0.0022 \\
2021\_10\_19 & 1-AI & 293 & 0.203 & 0.0001 \\
2021\_10\_19 & 1-AI & 3218 & 0.246 & 0.0001 \\
2021\_11\_08 & 1-AI & 2503 & 0.255 & 0.0002 \\
2021\_11\_08 & 1-AI & 2580 & 0.271 & 0.0001 \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Spike Prediction Limitation}: The models struggle to predict the sharp activations (spikes) that are clinically relevant. Increasing complexity from AR to ARIMA or TVAR provided little improvement.
    ...
    \item \textbf{Clinical Application}: By ranking electrodes by NMSE, we identified candidate ablation sites, regions where the AR model fails most severely.
\end{enumerate}
